<img width="1280" height="640" alt="ccbanner" src="https://github.com/user-attachments/assets/f06d58d1-a503-4d4d-b7b6-cca4747645f7" />
...
The assistant replies with the minimum file/function list it needs, ending with `END`.
Type `CC`.
...
Paste that into your assistant chat.
The assistant replies with a `patch.txt` artifact containing raw `CC-REPLACE` blocks.
...
DIR
```
Runs the project-tree export.
```text
CC
```
Runs the source/function export.
...
Version cache is enabled by default.
Successful writes create snapshots under:
```text
...
Older versions placed scripts directly in the project root.
After moving to `contextcontrol/`, you can remove old root-level helper files with:
```powershell

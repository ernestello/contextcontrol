...
private void OnProjectTreeRowPointerPressed(object? sender, PointerPressedEventArgs e)
{
    ...
    var isPrimaryDoubleClick = point.Properties.IsLeftButtonPressed && e.ClickCount == 2;

    ViewModel.SelectTreeRow(row, false);

    if (isPrimaryDoubleClick && row.Node is { IsFile: true } node)
    {
